...
# Split the line, set initial polarity as positive/negative, output to file
...
def create_lexicon(fin):  
	lexicon = []
	with open(fin, 'r', buffering=100000, encoding='latin-1') as f: # buffering for large files
		try:
			counter = 1
			content = ''
			for line in f:
				counter += 1
				if (counter/2500.0).is_integer(): # take 2500 bytes of the buffer at one time
					tweet = line.split(':::')[1]
					content += ' '+tweet
